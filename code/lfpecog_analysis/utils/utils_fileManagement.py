...
def get_project_path(
    ...
    if not extern_HD:
        ...
        while not path.endswith("dyskinesia_neurophys"):
            ...
            while_count += 1
            if while_count > 20:
                return 'repo folder "dyskinesia_neurophys" not found'
...
def get_onedrive_path(
    ...
    """
    folder_options = ["onedrive", "figures", "bids_rawdata", "data"]
    ...
    onedrive_f = [
        ...
        if logical_and("onedrive" in f.lower(), "charit" in f.lower())
    ]
...
def get_beta_project_path(folder: str = "beta"):
    """
    Device and OS independent function to find
    ...
    """
    folder = folder.lower()
    folder_options = ["beta", "figures", "results", "data"]
    if folder.lower() not in folder_options:
        raise ValueError(
            f"given folder: {folder} is incorrect, "
            f"should be {folder_options}"
    ...
    assert exists(betapath), f"created path {betapath}" " does not exist"
...
def load_ft_ext_cfg(
    ...
    if isinstance(cfg_folder, str):
        ...
        if USER == "jeroen":
            json_path = join(
                get_onedrive_path("data"), "featureExtraction_jsons", cfg_fname
            )
        elif USER.lower() == "timon":
            # if while loop unsuccesful
            json_path = join(
                get_project_path("data", USER=USER), "meta_info", cfg_fname
            )
...
def save_dfs(df, folder_path: str, filename_base: str):
    ...
    save(join(folder_path, f"{filename_base}_timeIndex"), df.index.values)
    # save column-names as csv
    with open(
        join(folder_path, f"{filename_base}_columnNames.csv"), "w"
    ) as csvfile:
        write = writer(csvfile)
        write.writerow(list(df.keys()))
        csvfile.close()

    print(f"\n\tDataFrame ({filename_base}) " f" is stored to {folder_path}\n")
...
class mergedData:
    ...
    def __post_init__(
        ...
        if self.sub == "010":
            new_tasks = correct_tasks_010(
                colnames=self.colnames, data_arr=self.data
            )
...
def correct_acc_class(acc):
    ...
    if list(acc.times[:3]) == ["0", "1", "2"] or list(acc.times[:3]) == [
        0,
        1,
        2,
    ]:
...
def correct_tasks_010(data_class=False, colnames=False, data_arr=False):
    ...
    for tsk, t1, t2 in task_timesTuples:
        sel = logical_and(time_min > t1, time_min < t2)
        task_col[sel] = tsk
...
def get_avail_ssd_subs(
    ...
    ssd_path = join(
        ...
        ssd_folder,
        DATA_VERSION,
        f"windows_{WIN_LEN}s_" f"{WIN_OVERLAP}overlap",
    )
    if USER == "timon":
        ssd_path = join(
            get_project_path("results", USER=USER),
            "features",
            ssd_folder,
            DATA_VERSION,
            f"windows_{WIN_LEN}s_" f"{WIN_OVERLAP}overlap",
        )
    SUBS = list(set([name.split("_")[1] for name in listdir(ssd_path)]))
