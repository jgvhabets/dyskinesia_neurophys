...
from prep_movement_psd_analysis import select_taps_out_window, get_tap_times
...
def get_windows(
    ...
    winLen_sec=180,
    part_winOverlap=0.5,
    min_winPart_present=0.66,
    EXCL_TAPS=False,
    DATA_VERSION=None,
    sub=None,
    remove_nan_timerows: bool = True,
    movement_part_acceptance: float = 1.0,
    ...
    if EXCL_TAPS:
        ...
        tap_starts, tap_ends = get_tap_times(
            sub=sub,
            return_in_secs=True,
    ...
    if only_ipsiECoG_STN:
        ...
        if ecog_side == "R":
            ...
            contra_side = "R"
        del_sel = np.array(
            [f"_{contra_side}_" in c and "LFP" in c for c in arr_keys]
        )
        data = data[:, ~del_sel]
    ...
    if sum("ECOG" in c.upper() for c in arr_keys) == 12:
        if sum("SMC_AT" in c.upper() for c in arr_keys) == 6:
            ...
            if isinstance(arr_keys, list):
                arr_keys = np.array(arr_keys)
            print(
                f"...during get_windows(), sorted out doubles: {arr_keys[sel_dbl_ecog]}"
            )
    ...
    if remove_nan_timerows:
        ...
        good_cols = [
            sum(isna(data[:, i])) < max_nan_samples
        ...
        print(
            [
                f"{arr_keys[i]}: {sum(isna(data[:, i]))} NaNs"
                for i in range(data.shape[1])
            ]
        )
    ...
    tStart = round(times[0] / winLen_sec) - (winLen_sec * 0.5)
    ...
    secs_present = times[
        :: int(fs)
    ]  # get roughly every second present in data
    ...
    for win0_sec in np.arange(tStart, times[-1], sec_hop):
        ...
        if isinstance(data, DataFrame):
            ...
            if wintemp.shape[0] < min_winPart_samples:
                if verbose:
                    print("\tskipped based on NaNs", win0_sec)
                continue

            if verbose:
                print(f"\tINCLUDED shape is {wintemp.shape}")
        ...
        if EXCL_TAPS:
            win_tap_bool = select_taps_out_window(
                wintimes, tap_starts, tap_ends
            )
            ...
            if any(win_tap_bool == False):
                ...
                print(
                    f"\tin win@ {win0_sec}s: tap-removed: {sum(~win_tap_bool) / fs} seconds"
                )

        # NaN-PAD WINDOWS NOT FULLY PRESENT (to fit 3d-array shape)
        if wintemp.shape[0] < nWin:
            rows_to_pad = nWin - wintemp.shape[0]
            nanpad_arr = np.array([[np.nan] * wintemp.shape[1]] * rows_to_pad)
    ...
    print(
        f"...removed # {sum(~np.array(arr_bool))} windows bcs of length of end of windowing"
    )
    print(
        f"STARTTIMES length ({len(arr_times_sec)}) vs "
        f"ARRAYS length ({len(win_array)})"
    )

    if return_as_class:
        windows = windowedData(
            data=win_array, fs=fs, keys=arr_keys, win_starttimes=arr_times_sec
        )
        return windows
...
def window_to_epochs(
    ...
    if mne_format:  # requires n_epochs, n_channels, n_samples
        epoched_array = np.array([e.T for e in epoched_array])

    assert ~isna(epoched_array).any(), "resulting epoched 3d-array has NaN's"
...
def create_mne_epochs(
    ...
    if pick_only_ephys:
        ephys_sel = [
            np.logical_or("ECOG" in col, "LFP" in col) for col in ch_names
        ]
        ch_names = list(compress(ch_names, ephys_sel))
        epoched_windows = [e[:, ephys_sel, :] for e in epoched_windows]

    # create obligatory mne-info
    mne_info = create_info(
        ch_names=list(ch_names), sfreq=fs, ch_types=["eeg"] * len(ch_names)
    )
...
def get_noNanSegm_from_singleWindow(
    ...
    if win_times.size == 1:
        ...
        if len(windat) != len(win_times):
            raise ValueError("Unequal shapes of windat and win_times")
    ...
    if timing:
        seg_times = seg_times[~np.array(nanrows)]

    if timing:
        return segdat, seg_times
...
def get_epochedData_perChannel(
    ...
    segLen_sec: float = 0.5,
    part_segmOverlap: float = 0.5,
    ...
    if return_3d_segmData:
        ...
        epoched_channel = epochedChannel(
            data=segDat, segmTimes=segTimes, winTimes=windows.win_starttimes
        )
        ...
        epoched_channel = epochedChannel(
            data=np.array(newDat), winTimes=np.array(newTimes)
        )
...
class epochedData_multipleChannels:
    ...
    def __post_init__(
        ...
        for ch in self.channels_incl:
            ...
            setattr(self, ch, epochedChannel)
